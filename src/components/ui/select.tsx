...
import { Check, ChevronDown, ChevronUp, TriangleIcon } from 'lucide-react'
...
>(({ className, children, iconClassName, ...props }, ref) => (
  <SelectPrimitive.Trigger
    ref={ref}
    className={cn(
      'flex h-10 gap-1 w-full items-center justify-between rounded-md border border-slate-200 bg-white px-3 py-2 text-sm ring-offset-white placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-slate-950 focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 [&>span]:line-clamp-1 dark:border-slate-800 dark:bg-slate-950 dark:ring-offset-slate-950 dark:placeholder:text-slate-400 dark:focus:ring-slate-300',
      className
    )}
    {...props}
  >
    ...
    <SelectPrimitive.Icon asChild>
      <TriangleIcon
        className={cn(
          'h-2 scale-y-75 w-2 rotate-180 fill-gray dark:fill-white stroke-none opacity-50',
          iconClassName
        )}
      />
